...
class BNNExperiment(object):
    def __init__(self, step_size=1e-3, batch_size=100, hidden_dim=50, a0=1.0, b0=0.1, n_points=20):
        ...
        self.h = 0
        self.iter = 0
        self.scale_x = np.max(np.abs(self.train_x), axis=0)
        self.scale_y = np.max(np.abs(self.train_y), axis=0)
    ...
    def get_data(self, test_size=0.1, valid_size=0.1):
        data = pd.read_csv('UCI CBM Dataset/data.txt', sep='   ', header=None)
        data = data.values
    ...
    def generate_dlnp_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.origin_ln_likelihood = -n_data / 2 * (tf.math.log(2 * np.pi) - self.ln_gamma) - tf.exp(self.ln_gamma) / 2 * tf.reduce_sum(tf.pow((self.y - self.f)*self.scale_y, 2))
            ...
            with self.graph.as_default():
                with tf.Session() as sess:
                    result = sess.run(self.result, feed_dict={
                        self.x: self.batch_x/self.scale_x,
                        self.y: self.batch_y/self.scale_y,
                        self.w1: w1,
                        self.b1: b1,
                        self.w2: w2,
                        self.b2: b2,
    ...
    def get_rmse(self, x, points):
        ...
        for i in range(points.shape[0]):
            ...
            losses.append(np.sqrt((((result - self.test_y)*self.scale_y) ** 2).mean()))
        return min(losses)
    ...
    def get_ll(self, x, y, points):
        ...
        for i in range(points.shape[0]):
            ...
            with self.graph.as_default():
                with tf.Session() as sess:
                    result = sess.run(self.origin_ln_likelihood, feed_dict={
                        self.x: x/self.scale_x,
